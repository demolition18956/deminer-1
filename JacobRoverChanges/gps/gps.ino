...
const byte NUMBEROFCORNERS 		= 4;
const byte NUMBEROFWAYPOINTS 	= 81;
const byte MINEFIELDWIDTH		= 9;
...
const float deg2rad				= 0.017453292;	//this is the most accurate a 32bit float can represent pi/180
const float rad2deg				= 57.29577951;	//same for this value
const float lat2meter			= 110935.42;
const float lon2meter			= 91598.80;
...
gpsData myGpsData;
boundaryCorner myBoundaryCorner;
waypoint myWaypoint;
roverData myRoverData;

void setup() {
  // put your setup code here, to run once:
  Serial.begin(9600);                 //serial0 connection to the computer
  Serial2.begin(9600);                 //serial2 connection to the other MEGA
  Serial3.begin(115200, SERIAL_8N1);  //serial3 connection to the GPS Module
  
  //BeginningSetup(&myGpsData, &myBoundaryCorner, &myWaypoint, &myRoverData);
  GPS_Setup(&myGpsData);
  delay(1000);
}
...
void loop() 
{
  if (millis() - lastMillis >= 2*1000UL)
  {
    ...
    Serial.println("waiting for Serial 2 data");
  }
  ...
  {   
      ...
      if(inByte == GPSDIRECTION)
      {
        Serial.println("Getting GPSDIRECTION");
        GetGenericData(GPSDIRECTION, &myGpsData);
        Serial2.write((byte)myGpsData.gps_Course);
        Serial.println(myGpsData.gps_Course);
        //float outByte = myGpsData.gps_Course;
        //serialFloatPrint(myGpsData.gps_Course);
      }
  }
}
...
{
  while(ptr_gps->Health.stat != 3)    //this waits for a GPS lock on enough satellites
  {
    Serial.println("waiting for GPS Health");
	 	GetGenericData(GPSHEALTH, ptr_gps);
    Serial.println(myGpsData.Health.stat);
  }
...
void WaypointSetup(waypoint* wpoint, boundaryCorner* bptr)
{
  ...
  for(i = 1; i < NUMBEROFWAYPOINTS; i++)
  {
    ...
    column = (i-1) % MINEFIELDWIDTH;
		...
		/*use this to print out debug information for the waypoints
		if(debug)
		{
			Serial.print("Direction between Boundary Zero and One is ");
			Serial.println((direction*rad2deg), 5);
			Serial.print("Waypoint ");
			Serial.print(wpoint[row*9 + column].id, 5);
			Serial.print(" position north of Boundary Zero = ");
			Serial.println(wpoint[row*9 + column].distanceFromHomeNorth, 5);
			Serial.print("Position East From Boundary Zero = %0.3d");
			Serial.println(wpoint[row*9 + column].distanceFromHomeEast, 5);
		}*/
  }
}
